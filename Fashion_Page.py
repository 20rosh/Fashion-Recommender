    ...
    with st.spinner("Extracting features and finding recommendations..."):
        ...
        if features.shape[0] != feature_list[0].shape[0]:
            ...
            for i in range(top_k):
                ...
                # If full path, take only filename
                rec_img_path = os.path.basename(rec_img_path)
                rec_img_path = os.path.join("images", rec_img_path)  # Adjust if images/ is the directory
                ...
                try:
                    ...
                    st.warning(f"⚠️ Could not load image: {rec_img_path}")
